    ...
    async getDashboardStats() {
        ...
        const [totalPlans, activePlans, completedPlans, plansThisMonth] = await Promise.all([
            ...
            prisma_1.prisma.travelPlan.count({
                where: {
                    isDeleted: false,
                    status: { in: [client_1.TripStatus.PLANNING, client_1.TripStatus.UPCOMING, client_1.TripStatus.ONGOING] },
                },
            }),
            prisma_1.prisma.travelPlan.count({
                where: {
        ...
        const [pendingRequests, acceptedRequests, rejectedRequests] = await Promise.all([
            prisma_1.prisma.travelRequest.count({ where: { status: client_1.RequestStatus.PENDING } }),
            prisma_1.prisma.travelRequest.count({ where: { status: client_1.RequestStatus.ACCEPTED } }),
            prisma_1.prisma.travelRequest.count({ where: { status: client_1.RequestStatus.REJECTED } }),
        ]);
        return {
            users: {
                total: totalUsers,
