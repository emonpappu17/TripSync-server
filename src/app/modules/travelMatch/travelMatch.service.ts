...
class TravelMatchService {
     ...
     */
    async getMyMatches(userId: string, options: IOptions) {
     ...
     */
    async getMatchesByPlanId(planId: string, userId: string) {
     ...
     */
    async deactivateMatch(matchId: string, userId: string) {
        const match = await prisma.travelMatch.findUnique({
            where: { id: matchId },
        });
        ...
        if (match.user1Id !== userId && match.user2Id !== userId) {
            throw new ApiError(
                ...
                'You do not have permission to deactivate this match'
            );
        }
     ...
     */
    async checkMatch(planId: string, userId: string, otherUserId: string) {
     ...
     */
    async getMatchStatistics(userId: string) {
